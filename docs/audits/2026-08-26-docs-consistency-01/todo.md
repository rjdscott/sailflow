...
- [ ] **M-07** (P2, S) `pctPolar` tier under the kite: route through `tierFor` (recommended) or document.
- [ ] **M-06** (P2, S) Dock regret tier: B (docs) or A (code).
- [ ] **M-05** (P2, S) Forecast pmf floor: 5 % of peak (as coded) or 5 % probability.
- [ ] **M-04** (P2, S) Dock `T*(w)` reference: document the fold-in or restore the pure grid.
- [ ] **M-11**, **M-12** (P2, S) `--range-*` contrast rows or decorative wording; third palette block in `contrast_check`.
- [ ] **M-09**, **M-10**, **M-20** (P2, S) report commit stamp; `boat.ts` ceiling comment; research doc 02 §6 clew-rise cell.
- [ ] **M-25b** (P2, M) Plan view draws the bulged leech.
