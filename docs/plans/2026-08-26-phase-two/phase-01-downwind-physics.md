...
- [ ] Reproduce the miss in a test: TWS 14 `asym vmgDn` solves to 172° vs polar 146°; write the failing invariant first.
- [ ] Diagnose: sweep `optimal()` over TWA 120–180 at TWS 8/14/20 and plot VMG; establish whether the objective is flat (no gybe-angle optimum in the aero) or the optimum exists and the search misses it.
