...
- 2026-08-26 — Downwind default no longer demotes itself to tier C. Owner report (live site, reproduced locally): under `sailset: 'asym'` the app's own downwind default — `baseRaceDown.mainsheet` 15, boom ≈ 67° — returned `bsKt.tier === 'C'` and `pctPolar.tier === 'C'`, while a beat's mainsheet 60 returned B. The brief's diagnosis was that `sheetingDeviation` measured the main against the upwind ideal; **measured, that is not the cause**. `sheetingEffect` never reaches `tierFor` at all — `trimmed()` passes it `shapeToOrc(...).deltas` — and `sheetingDeviation` already caps its target at `awaDeg − 90`, which past ~106° AWA *is* an eased downwind reference: at 150° TWA the eased boom reads dev +17.2° (`clMul` 0.80), the best of the range, against +66.7° (0.52) for boom 19.6° and +81.8° (0.49) for a boom pinned at 6°. The real cause was one line up the pipe: `shape/toOrc.ts` measures `dTwistDeg` against `referenceShapes()`, and that reference was `baseRace()` — a beat's mainsheet — for **both** sail sets. Easing 60 → 15 opens the leech by `shape.sheetToTwist` 0.12 °/% × 45 % = 5.4° at the head, 2.19° at half height, for `shapeInfluence` 0.135 against `SHAPE_DEMOTE_THRESHOLD` 0.08. The demote rule (ADR 0006) was right; its datum was wrong downwind. Fix: `shape/base.ts` gains `baseRaceDown()` and `referenceShapes(boat, sails, sailset)`, and `solve/optimal.ts` seeds its downwind rows from the same datum (without that half, every VPP asym row carried one whole sheet ease of deviation and invariant 18 read C). `shape/sheeting.ts` is **unchanged** except a comment naming the `awaDeg − 90` cap as the downwind reference, so the next reader does not re-derive the same wrong theory; the `notSolved` comment in `optimalTrim.ts` is corrected — it claimed `sheetingEffect` was "the sheet's only route into a downwind solve", and the shape layer's leech twist is the other, which is the route that was doing the damage. Measured after: mainsheet 15 → B/B, 30 → B, 45/60 → C, 100 (pinned on a run) → C, i.e. the demote now fires for genuine over-trim and a beat's sheet under a kite, which is the intent. `pnpm validate` **verdict unchanged** — same two rows FAIL, TWS 14 `jib vmgUp` 5.8 %/1.8° byte-untouched and TWS 14 `asym vmgDn` 15.1 %/25.5° byte-untouched (heel 1.9 → 2.0, toward the polar's 11.7). Both held-out asym rows (TWS 8 and 14 `vmgDn`) did not move at all, so the brief's stop rule was not triggered. Two *fitted* asym rows did: TWS 16 `vmgDn` bs err 4.1 % → 5.3 % but twa err 4.8° → 3.9°, and TWS 20 `vmgDn` bs err 8.1 % → 8.3 % but twa err 0.7° → 0.6° — bs worse, angle better, on rows whose calibration was fitted against the old datum. Not tuned; a refit would absorb it. `pnpm golden`: **every jib case byte-identical** in all three files (0 of 42), 23 asym cases moved — main twist sections and the aero/rig/instrument values downstream of them, plus `race.mainsheet` and `race.backstay` on the polar points; no `tier` field moved anywhere, so the corpus stays B under the kite.
- 2026-08-26 — #86's "`trimmed()` does not converge at the downwind default" follow-up closed, not as a convergence bug. Reproduced only with the agent's probe, which built its `Condition` with `as any` and no `crewKg`/`seaState`; with a full condition every downwind case converges in 3–7 iterations (swept mainsheet 15–100, crew 200–400 kg, sea 0–4, TWS 6–20). `crewKg: undefined` made the righting moment NaN, so Newton bailed on iteration 1 of each seed and returned the seed-table speed with `converged: false` — a silent wrong answer. `solveEquilibrium` now throws `non-finite condition <fields>` up front (SciPy `root` / MATLAB `fsolve` contract: bad inputs raise, they do not "not converge"); the worker already forwards thrown errors as `type: 'error'`. Regression test in `solve.test.ts`.
- 2026-08-26 — Boat-hash follow-up (#84) closed: `boatHash()` now covers the boat data the solver reads and leaves out `provenance` and `sources` (only `boat/validate.ts` looks at them), so a note edit no longer fails the golden test or forces a corpus regen. `validation/hash.test.ts` pins both halves — a note edit does not move it, a geometry edit does. Corpus regenerated: hash line only, no value moved.
