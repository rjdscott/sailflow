...
- 2026-08-26 — Deferred at close-out (v0.3.0). The hold-out gate (ADR 0012) stands at **FAIL — 8/10**: TWS 14 `jib vmgUp` 5.8 % / 1.8° and TWS 14 `asym vmgDn` 15.1 % / 25.5° (the model runs 172° where the polar gybes at 146°). The upwind residual is a calibration-envelope problem; the downwind one is a model-structure problem — the asym aero has no VMG-down mode switch (soak vs plane) and no gybe-angle optimum of its own. Both are picked up as phase 01 of `docs/plans/2026-08-26-phase-two/`. Everything else this phase promised (solver, calibration, validation harness, golden corpus, `pnpm validate` exit code) shipped and is enforced in CI.
