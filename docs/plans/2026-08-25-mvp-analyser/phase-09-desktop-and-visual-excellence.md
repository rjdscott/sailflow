...
- 2026-08-25 — Owner review: telltales too static, traveller inverted, no main telltales. PRs #22–#24: telltales now read local angle of attack (AWA − sheeting angle − ½ twist at height) against assumed targets (jib entry 12°, main leech 8°); main leech telltales at ¾ and mid-leech; `boomAngle` sign fixed so + traveller = up/windward (label says so). Live probe on Pages: base all streaming; jib 100 % → all four stall; jib 35 % → all lift; traveller ±60 → boom 99°/108° on screen; main sheet 100 + traveller +50 → both leech telltales stall. Golden corpus regenerated (#23, label changed the boat hash).
