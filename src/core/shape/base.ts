...
/**
 * Race base: the mid-range trim for a boat sailing upwind in the guide's base
 * wind band, read from `data/boats/j70.json` (`baseRace`, prov: assumed —
 * the guide publishes "Firm"/"Snug"/"5-6 holes showing", not percentages).
 *
 * The JSON is the single source: Race mode's default trim in
 * `src/ui/stores/conditions.svelte.ts` reads the same block, so the datum the
 * shape deltas are measured against and the trim the sliders start on cannot
 * drift apart again (cockpit phase 05).
 */
