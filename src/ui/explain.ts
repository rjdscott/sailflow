...
export const READOUT_EXPLAIN: Record<string, string> = {
  ...
  tws: 'True wind speed: the wind over the water, in knots, which is the wind a forecast quotes and the wind the tuning guides band their settings by. It is not what the masthead instrument reads once you are moving — that is apparent wind, tighter and stronger upwind, softer and freer downwind. Everything on this screen is solved for the speed set here, so it is the first thing to set and the last thing to argue with.',
  twa: 'True wind angle: the angle between the true wind and the bow, so 0° is head to wind and 180° is dead downwind. Upwind a smaller angle is pointing higher, on a run a bigger one is sailing deeper. Drag the arrow on the rose, or use the arrow keys — one degree a press, five with shift. The angle is yours to set except when a point-of-sail preset solves for the VMG-optimal one, which is why the preset stops looking pressed as soon as you move away from the angle it chose.',
  sea: 'Sea state is added resistance from waves: the model adds a drag term on top of the flat-water hull, and steeper water costs more of it, which is why the same trim is slower in chop than on a mirror. Tier B — the shape of the penalty is right and its size is calibrated rather than measured, and it is modelled in `core/hydro/waves.ts`. It changes nothing about how the sails are drawn: it is a resistance the boat has to push through, not a wind that arrives differently.',
  crew: 'Crew weight, in kilograms, all of it assumed on the rail: it is the righting moment the boat carries against the sails’ heeling moment, so more of it holds the boat flatter and lets the rig stay powered further into the breeze. The class limit is the range the stepper allows. Where the crew sits fore and aft is *not* modelled — the Helm panel logs it and says so.',
  sailset:
    'Which headsail is up: the jib, or the gennaker on the sprit. It changes the sail plan the solver builds, the polar column the % polar reading is measured against, and which panel the headsail slot shows. A point-of-sail preset flips it for you — you cannot fly a kite on a beat, and the beat’s mainsheet cannot come downwind with you, so hoisting the kite also eases the main to the shroud.',
